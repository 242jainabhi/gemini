...
class EnvironmentController(Resource):
    ...
    def post(self):
        ...
        response = EnvironmentService().add_environment(env_obj)
        if response:
            data['id'] = env_obj.id
    ...
    def put(self, env_id):
        ...
        data = request.get_json()
        print(data)
        return EnvironmentService().update_environment(env, data)

    def delete(self, env_id):
        env_obj = EnvironmentService().get_environment(env_id)
        return EnvironmentService().delete_environment(env_obj)


class EnvironmentsController(Resource):
    def get(self):
        envs = EnvironmentService().get_all_envs()
        resp = {}
        for env in envs:
